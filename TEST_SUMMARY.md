...
## Current state: 423 tests passing — mock by default, real databases via docker and CI
...
| PropertyCapture | 36 | Recording-Proxy selector capture: property/projection/aggregate classification, nested and computed selectors as `opaque`, `g.key`, `average` alias |
| Injection | 46 | Identifier/operator validation: closed operator set, metadata-checked columns, having-expression shapes, end-to-end rejection of injection payloads through `where()`/`orderBy()`/`having()` |
| QueryFilterCoverage | 22 | Structured filters proven on every read path (count, pagination per dialect, all(), aggregates, select, find, groupBy incl. having placeholder ordering); legacy-lambda limitations pinned |
...
| IdentityMap | 9 | Same tracked instance on repeated loads (find/toList), local modifications survive re-query, eviction on delete/clear, asNoTracking excluded, per-context isolation |
