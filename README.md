...
- **Global Query Filters (structured form)**: `hasQueryFilter({ property, operator, value })` conditions are **translated to SQL WHERE clauses** on every query path (`toList`, `find`, `where()` chains, aggregates, projections, `groupBy()`, and the queries that load `include()`d relations), so filtered rows never leave the database. Bypass per-query with `ignoreQueryFilters()`, which propagates to includes. See [Global Query Filters](#global-query-filters)
...
Deletes carry the same check. `DELETE FROM t WHERE id = $1 AND version = $2`
throws the same concurrency error when it affects no rows, so deleting a row
another user already changed or deleted fails loudly instead of silently
doing nothing.
...
The token is incremented client-side, in the `UPDATE ... SET version = $1`
that carries the write, so it has to be a number. A non-numeric token throws
at save time rather than writing a meaningless value into the column.

Database-generated tokens (`timestamp`, `rowversion`, a GUID regenerated by a
trigger) are **not supported**: nothing reads the new value back after the
write, so the entity would hold a stale token and the next save would fail.
...
**Token type:** an integer counter (rowVersion = 1, 2, 3...). Timestamp and
GUID tokens are not supported, see above.
